...
// =====================================================================
// Book별로 저장 키를 생성하는 헬퍼 함수
function getStorageKeys() {
    const currentBook = currBookName || "DefaultBook";
    // 진행 상황은 전체(UID)로 관리하고, 개별 북의 챕터 리스트는 북별로, 설정 정보는 전역으로 관리합니다.
    return {
        studyData: studySaveName + '_studyData', // 전체 진도 정보 (UID 식별)
        myChapterList: studySaveName + '_' + currentBook + '_myChapterList',
        myChapterInfo: studySaveName + '_myChapterInfo' // [고정] 어떤 북을 보든 설정 위치를 찾을 수 있게 고정함
    };
}
// =====================================================================
...
let myChapterInfo = {};  // 현재 공부하는 chapter 정보를 Local Storage에 저장. object 파일임.
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    const savedValue = this.localStorage.getItem(keys.studyData);

    if (savedValue) {  // local Storage에 값이 있으면 실행 
        loadLocalStorage();
    } else {
        // 저장된 값이 없으면 초기화 함수를 호출합니다.
        initializeStudyData(1); // 1: 전체 초기화
    }
...
async function initializeStudyData(value) {
    ...
    if (initializeValue !== "cancel") {
        ...
        if(initializeValue == 1){
            // [수정] '모든 Chapter 초기화'를 '현재 Book의 모든 Chapter 초기화'로 변경
            // 전체 studyData를 리셋하는 loadJsonFromFile() 대신, 현재 Book에 해당하는 항목만 초기화합니다.
            if (!currBookName) { // currBookName이 아직 설정되지 않은 경우 (예: 최초 로딩 직후)
                // 이 경우, 전체 JSON을 다시 로드하여 기본 상태로 만듭니다.
                ...
                for (let i = 0; i < studyData.length; i++) {
                    if (studyData[i].book_name === currBookName) {
                        ...
                        studyData[i].group = 1; // 그룹은 1로 초기화
                        studyData[i].test_count = 0;
                        // chapter, chapter_name, chapter_orig는 유지 (합치기/원복 기능에 의해 변경된 상태 그대로)
                    }
                }

                // 현재 Book에 해당하는 myChapterList 메타데이터를 LocalStorage에서 제거하고 메모리에서도 초기화
                const keys = getStorageKeys();
                localStorage.removeItem(keys.myChapterList);
                myChapterList = {}; // 메모리에서도 초기화

                // 현재 Book의 첫 번째 항목으로 포커스를 맞춥니다.
                currStudyDataNum = studyData.findIndex(item => item.book_name === currBookName) || 0;
                currChapterName = studyData[currStudyDataNum].chapter_name;

                await checkChapter(); // 챕터 목록 갱신 (현재 Book 기준)
                myChapterListInfoMake(); // myChapterList 재구성 (기본값으로)
                await groupReassign(); // 그룹 재할당 (히스토리 없는 상태로 5000으로 설정됨)
                currGroupNum = 1; // 그룹 번호 1로 설정
                loadValue(currStudyDataNum); // 화면 업데이트
                saveLocalStorage(); // 변경된 상태 저장
                showPopup(`현재 Book '${currBookName}'의 모든 Chapter를 초기화했습니다.`);
            }
        }
...
async function loadJsonFromFile() {
    ...
    // 기존에 저장된 Local Storage 정보를 모두 지운다. 
    const keys = getStorageKeys();
    localStorage.removeItem(keys.studyData);
    localStorage.removeItem(keys.myChapterList);
    localStorage.removeItem(keys.myChapterInfo);
...
function loadValue(currStudyDataNum) {
    ...
    let displayTitle = `<div style="font-size: 0.8em; color: #666;">Foreign Speaking v0.02</div>`;
    displayTitle += `<div><span class="mychaptercolor" onclick="changeBook()">${bookName}</span> - <span class="mychaptercolor" onclick="changeChapter()">${chapterName}</span></div>`;
...
async function changeStatusToYes() {
    ...
    saveLocalStorage();  // 현재 값을 메모리에 저장하기 
...
// 브라우저 로칼 저장소에 각종 학습 데이터 및 변수 저장
function saveLocalStorage() {
    const keys = getStorageKeys();
    // object를 저장할때는 obj => json으로 변환해서 저장한다. 
    // 학습 데이터 정보 저장 => Quota 초과 에러 방지를 위해 progress 정보만 저장함.
    if (studyData) {
        const progressData = studyData.map(item => ({
            ...
            // [추가] chapter, chapter_name, chapter_orig 정보도 저장하여 재시작 시 유지되도록 함
            chapter: item.chapter,
            chapter_name: item.chapter_name,
            chapter_orig: item.chapter_orig
        }));
        localStorage.setItem(keys.studyData, JSON.stringify(progressData));
    }
    // chapter 정보 저장 
    // currChapterName = currChapterName;
    ...
    myChapterInfo.showScriptModeKor = showScriptModeKor;
    ...
    const jsonMyChapterInfoString = JSON.stringify(myChapterInfo);
    localStorage.setItem(keys.myChapterInfo, jsonMyChapterInfoString);
    ...
    const jsonMyChapterString = JSON.stringify(myChapterList);
    localStorage.setItem(keys.myChapterList, jsonMyChapterString);
}

// 브라우저 로칼 저장소에서 불러오기 
async function loadLocalStorage() {
    // 초기 호출 시 기본 키로 Info 로드 시도
    let keys = getStorageKeys();
    // obj를 가져올때에는 json => obj로 다시 변환하여 가져오면 된다.
    // 현재북 정보 및 Chapter 설정값 가져오기
    const jsonMyChapterInfoString = localStorage.getItem(keys.myChapterInfo);
    if (!jsonMyChapterInfoString) return; // 저장된 정보가 없으면 중단

    myChapterInfo = JSON.parse(jsonMyChapterInfoString);
    ...
    const progressString = localStorage.getItem(keys.studyData);
    const progressData = progressString ? JSON.parse(progressString) : []; // progressData가 null일 경우 빈 배열로 초기화

    studyData = baseData.map(baseItem => {
        const progress = progressData.find(p => p.uid === baseItem.uid);
        if (progress) {
            // 저장된 진행 상황과 기본 데이터를 병합
    ...
    // currStudyDataNum이 복구되었으므로, 해당 Book에 맞는 Chapter 리스트 키를 생성합니다.
    keys = getStorageKeys();

    // Chapter List 가져오기 
    const jsonMyChapterString = localStorage.getItem(keys.myChapterList);
    myChapterList = jsonMyChapterString ? JSON.parse(jsonMyChapterString) : {}; // myChapterList가 null일 경우 빈 객체로 초기화
    ...
    countYesInChapter(currChapterName)  // 전체 암기 완료한 값을 찾는다. 
...
// 브라우저 메모리에 저장되어 있는 특정 문자열로 시작하는 키 삭제 함수
function removeLocalStorage(prefix) {
    // localStorage의 모든 키를 배열로 가져옴
    const keys = Object.keys(localStorage);

    // 각 키를 순회하면서
    keys.forEach(key => {
        // 키가 지정한 접두사(prefix)로 시작하면
        if (key.startsWith(prefix)) {
            // 해당 키를 localStorage에서 삭제
